<?php
   ...
   if ($_SERVER["REQUEST_METHOD"] == "POST") {
      ...
      if (empty($name_err) && empty($name_desc_err) && empty($price_err) && empty($count_err) && empty($image_err) && empty($name_type_err)) {
         ...
         // Sửa loại sản phẩm
         if ($thao_tac == "Sửa") {
            $update = DP::run_query("UPDATE sanphams SET ten_san_pham = ?, admin_id = ?, loai_san_pham_id = ?,mo_ta_san_pham = ?, so_luong = ?,don_gia = ?, hinh_anh = ? WHERE id = ?", [$name, (int)$_SESSION["id"], (int)$name_type, $name_desc, (int)$count, (int)$price, $image, (int)$id], 1);
            if ($update) {
